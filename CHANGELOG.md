...
- **peer 下限抬到 `ice-render@^2.18.0`**：那一版是**事件系统改版**（事件沿组件树冒泡、
  `ICEEvent` 的 W3C 方法真生效、事件名类型化）。本包生成的表单/控件最终由引擎渲染与交互，
  事件语义必须与渲染语义同版。

  仍停在 2.14~2.17 的宿主装本版本会得到 npm 的 peer 冲突提示 —— 有意的。
- **组件清单同步上游 `ice-web-components@1.22.0`**（devDependency 一并对齐，`catalog/components.json`
  与 `SKILL.md §7` 由 `npm run catalog` 重生成）—— 清单描述的必须就是被测的那一份。
...
- **组件清单同步上游 `ice-web-components@1.21.0`**（devDependency 一并对齐，`catalog/components.json`
  与 `SKILL.md §7` 由 `npm run catalog` 重生成）—— 清单描述的必须就是被测的那一份。
